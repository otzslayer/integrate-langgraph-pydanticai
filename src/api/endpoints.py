...
async def invoke_agent(
    ...
    async def stream_generator():
        try:
                ...
                if kind == "on_chain_end":
                    if event["name"] == "sql_generator":
                        ...
                        if thoughts := data.get("thoughts"):
                            ...
                            await asyncio.sleep(0.01) # 클라이언트가 받을 수 있도록 잠시 대기
            ...
            logger.error("에이전트 스트리밍 중 오류 발생", error=str(e), exc_info=True)
            error_message = json.dumps({"type": "error", "data": f"An error occurred: {e}"})
